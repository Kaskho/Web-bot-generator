...
def call_grok(narrative: str, task: str) -> str:
    ...
    payload = {
        "model": GROK_MODEL,
        "messages": [
            {"role": "system", "content": "You are Grok: create content for meme coin website and bot."},
...
async def generate(
    ...
    if file:
        ...
        media_path = str(save_to.name)

    # buat konten AI
    site_copy = call_grok(narrative, "Generate website tagline, intro, and roadmap (plain text)")
    bot_json = call_grok(narrative, "Generate JSON with arrays: GREET_NEW_MEMBERS, HYPE, WISDOM, SCHEDULED_BUY, SCHEDULED_PUMP")
    try:
        bot_texts = json.loads(bot_json)
    except Exception:
        bot_texts = {
            "GREET_NEW_MEMBERS": [f"Welcome to {coin_name}!"],
            "HYPE": ["LFG! To the moon!"],
            "WISDOM": ["In chaos we trust."],
            "SCHEDULED_BUY": ["Buy the dip!"],
            "SCHEDULED_PUMP": ["Pump time!"]
        }

    # buat folder hasil (website + bots)
    from jinja2 import Template
    index_tpl = (TEMPLATE_DIR / "website_index.j2").read_text()
    html = Template(index_tpl).render(
        coin_name=coin_name, ticker=ticker, network=network,
        tagline=site_copy.splitlines()[0] if site_copy else "",
        intro=site_copy, roadmap=site_copy,
        pump_fun=pump_fun, x_url=x_url,
